jQuery(function ($) {
  var jobsBoard = {
    ...
    jobsPostsApiURL:
      "https://api.lever.co/v0/postings/webfx?v1&group=team&mode=json",
    ...
    getJobsPostsArrayFiltered: function (departmentId, locationId) {
      ...
      for (var a = 0; a < jobsPostsArray.length; a++) {
        ...
        if (
          (departmentId == "" || jobItem.department.id == departmentId) &&
          (locationId == "" || jobItem.location.id == locationId)
        ) {
    ...
    setDeparmentsDropdownChangeHandler: function () {
      jobsBoard.getDeparmentsDropdown().change(function () {
        ...
        jobsBoard
          ...
          .find(".no-open-positions")
          .addClass("hidden");
        var jobsPostsArray = jobsBoard.getJobsPostsArrayFiltered(
          departmentId,
          locationId
        );
    ...
    setupJobsPaginationList: function (jobsPostsArray) {
      ...
      jobsBoard
        .getJobsListContainer()
        .find(".open_post-item:not(.job-model)")
        .remove();
      jobsBoard
        .getJobsListContainer()
        .find(".no-open-positions")
    ...
    addJobsPostsInBoard(jobsPostsArray) {
      var jobsItemModel = $(
        jobsBoard.getJobsListContainer().find(".job-model.hidden").get(0)
      );
      jobsBoard
        .getJobsListContainer()
        .find(".open_post-item:not(.job-model)")
      ...
      for (var a = 0; a < jobsPostsArray.length; a++) {
        ...
        var applyBtn =
          '<a href="' +
          postItem.applyUrl +
          '" class="btn btn-green" target="_blank">APPLY NOW </a>';
        ...
        $(
          '<div class="open_post-item">' + jobsItemModel.html() + "</div>"
        ).insertBefore(jobsBoard.getPaginationControl());
        var lastJobAdded = $(
          jobsBoard.getJobsListContainer().find(".open_post-item:last").get(0)
        );
    ...
    setupJobsBoard: function (data) {
      for (var a = 0; a < data.length; a++) {
        ...
        var departmentId = jobsBoard
          .removeNonAlphanumericCharacters(department.title)
          .toLowerCase();
        jobsBoard
          .getDeparmentsDropdown()
          .append(
            '<option value="' +
              departmentId +
              '">' +
        ...
        if (
          department.postings !== undefined &&
          department.postings !== null &&
          department.postings.length > 0
        ) {
          for (var p = 0; p < department.postings.length; p++) {
            ...
            var commitment = "";
            if (
              postItem.categories !== undefined &&
              postItem.categories !== null
            ) {
              if (
                postItem.categories.location !== undefined &&
                postItem.categories.location !== null
              ) {
                locationName = postItem.categories.location;
                locationId = jobsBoard
                  .removeNonAlphanumericCharacters(locationName)
                  .toLowerCase();
                if (
                  jobsBoard
                    .getLocationsDropdown()
                    .find('option[value="' + locationId + '"]').length == 0 &&
                  (locationId == "remote" ||
                    locationId == "guatemala" ||
                    locationId == "southafrica" ||
                    locationId == "capetown" ||
                    locationId == "johannesburg" ||
                    locationId == "antiguagt" ||
              ...
              if (
                postItem.categories.commitment !== undefined &&
                postItem.categories.commitment !== null
              ) {
            ...
            if (
              ...
              var newJob = {
                ...
                createdAtFriendlyDate: jobsBoard.convertTimestampToHumanDate(
                  postItem.createdAt
                ),
              };
    ...
    setJobsToggleButtons: function () {
      jQuery(".open_post-item .card-toggle").click(function () {
        $(".open_post-item").find("i").removeClass("cic-minus-icon cic-plus").addClass("cic-plus");
    ...
    init: function () {
      jobsBoard.getJobsPosts(
        {},
        jobsBoard.onSuccessJobsPostsHandler,
        jobsBoard.onErrorJobsPostsHandler
      );
    },
  ...
  jQuery(".hubs-carousel").slick({
    ...
    prevArrow:
      '<button type="button" class="slick-prev slick-arrow"><i class="cic-long-arrow-left"></i></button>',
    nextArrow:
      '<button type="button" class="slick-next slick-arrow"><i class="cic-long-arrow-right"></i></button>',
    responsive: [
      {
        breakpoint: 1200,
        settings: {
  ...
  function setupPopup() {
    ...
    $("#cookieConsent")
      .find("#closeCookieConsent")
      .click(function () {
        var expirationDate = new Date();
        expirationDate.setTime(
          expirationDate.getTime() + 400 * 24 * 60 * 60 * 1000
        );
        document.cookie =
          "close-country-disclaimer=true;  domain=.webfx.com; expires=" +
